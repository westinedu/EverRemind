...
class Game(GeoModel):
  """A location-aware model for game entities.
  ...
  phone_number = db.StringProperty()
  locale_code = db.IntegerProperty()
  game_type = db.IntegerProperty()
  #game_level = db.IntegerProperty()
  #grades_taught = db.ListProperty(int)
  
  @staticmethod
  def public_attributes():
    """Returns a set of simple attributes on game entities."""
    return [
      'game_id', 'name', 'address', 'city', 'state', 'zip_code',
      'playernumber', 'phone_number', 'locale_code', 'game_type'
    ]
