...
function initUI() {
  ...
  for (var type in GAME_TYPES) {
    var option = $('<option value="' + type + '">' +
                   GAME_TYPES[type] + '</option>');
    $('#search-game-type').append(option);
  }
  ...
  $('#search-game-type').change(function() {
  	calcRoute();
...
function doGeocodeAndSearch() {
  ...
  geocoder.geocode({'address':$('#search-query').val()}, function(response,status) {
    if (status != google.maps.GeocoderStatus.OK || !response) {
      ...
      $('#search-query').val(response[0].formatted_address);
      //alert(response.Placemark[0].AddressDetails.Accuracy);

      var bounds = response[0].geometry.viewport;
      
      map.setCenter(bounds.getCenter());
      map.setZoom(15);
      
      var ZoomAccuracy;
      if( response[0].geometry.location_type == google.maps.GeocoderLocationType.ROOFTOP)
      ZoomAccuracy = 14;
       if( response[0].geometry.location_type == google.maps.GeocoderLocationType.RANGE_INTERPOLATED)
        ZoomAccuracy = 10;
        if( response[0].geometry.location_type == google.maps.GeocoderLocationType.GEOMETRIC_CENTER)
        ZoomAccuracy = 7;
         if( response[0].geometry.location_type == google.maps.GeocoderLocationType.APPROXIMATE)
         ZoomAccuracy = 5;
      var proximitySearch = (ZoomAccuracy >=
      ...
      commonOptions.schoolType = $('#search-game-type').val();
      ...
      if (proximitySearch) {
        doSearch(updateObject(commonOptions, {
          type: 'proximity',
          centerAddress: response[0].formatted_address,
...
function doSearch(options) {
  ...
  if (g_searchCenterMarker) {
    ...
    g_searchCenterMarker.setMap(null);
    g_searchCenterMarker = null;
  }
  
  
  if (options.type == 'proximity') {
  	var image = new google.maps.MarkerImage('/static/images/markers/arrow.png',
  ...
  var shadow = new google.maps.MarkerImage('/static/images/markers/arrow-shadow.png',
    ...
    g_searchCenterMarker = new google.maps.Marker({
    	position: options.center,
      shadow: shadow,
        icon: image,
      draggable: true,
      zIndex: 1000
      //zindex_changed: function(){ return 1000; }
    });
    ...
    g_searchCenterMarker.setMap(map);
  }
  ...
  if (options.schoolType) {
	...
	alert(searchParameters.lat)
	alert(searchParameters.lon)
	alert(searchParameters.maxresults)
	alert(searchParameters.maxdistance)
  ...
  g_currentSearchXHR = $.ajax({
    ...
    error: function(xhr, textStatus) {
      ...
      alert(responseObj.error.message)
...
function clearSearchResults() {
  if (g_searchResults) {
    ...
    for (var i = 0; i < g_searchResults.length; i++) {
      ...
      g_searchResults[i].marker.setMap(null);
    }
  }
